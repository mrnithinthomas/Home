...
agent any
	stages
	{
		...
		stage('Git Clone')
		{
			steps
			{
            		git branch: 'main', url: 'https://github.com/mrnithinthomas/Home.git'
			}
		}
		stage('Maven Build')
		{
			...
			steps
			{
				script
				{
                 		withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: '', contextName: '', credentialsId: 'kubesecretfile', namespace: '', serverUrl: '']])  
                 			{
                           		def podName = 'Home-pod'  // Replace with your pod name
                    			...
                    			if (podExists) 
						{
						    ...
						    //sh "kubectl apply -f manifest.yaml"
                       	    sh "kubectl apply -f Home-pod.yaml"
						    sh "kubectl apply -f Home-service.yaml"
						    // sh "minikube start"
		...
		//	steps
		//	{
		//	sh 'echo http://localhost:${Host_Port}/Home/'
		//	}
		//}
